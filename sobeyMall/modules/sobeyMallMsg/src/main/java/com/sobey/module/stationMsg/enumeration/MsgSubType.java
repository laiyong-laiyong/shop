...
public enum MsgSubType {
    ...
    DiscountConfirmation("0","15","折扣确认"),

    SystemAnnouncement("2","8","系统公告"),
    ActivityNotice("2","9","活动通知"),
    News("2","10","新闻动态"),
    PopularRecommendation("2","16","热门推荐");


    public static String getDesc(String subTypeCode){
        for (MsgSubType msgSubType : MsgSubType.values()) {
            if (msgSubType.getCode().equals(subTypeCode)){
                return msgSubType.getDesc();
            }
        }
        return "";
    }
     ...
     * @return
     */
    public static String getBasicTypeCode(String subTypeCode){
        for (MsgSubType msgSubType : MsgSubType.values()) {
            if (msgSubType.getCode().equals(subTypeCode)){
                return msgSubType.getBasicCode();
            }
        }
        return "";
    }

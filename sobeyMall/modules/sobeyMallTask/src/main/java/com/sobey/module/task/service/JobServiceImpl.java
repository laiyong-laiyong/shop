package com.sobey.module.task.service;

import com.sobey.module.task.job.AsyncJob;
import com.sobey.module.task.job.CornJob;
import org.quartz.*;
    ...
    public void addCronJob(String jobName, String jobGroup) {
        try {
            ...
            if (jobDetail != null) {
                ...
                //构建job信息
                jobDetail = JobBuilder.newJob(CornJob.class).withIdentity(jobName, jobGroup).build();
                //用JopDataMap来传递数据
//                jobDetail.getJobDataMap().put("taskData", "hzb-cron-001");

                //表达式调度构建器(即任务执行的时间,每5秒执行一次)
                CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule("0/30 * * * * ?").withMisfireHandlingInstructionIgnoreMisfires();
